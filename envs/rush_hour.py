...
def triangle(r0, c0, width, im_size, orient = 0):
    if orient==0:    #up
        rr, cc = [r0, r0 + width, r0 + width], [c0 + width//2, c0, c0 + width]
    elif orient==1:  #right
        rr, cc = [r0 + width//2, r0, r0 + width], [c0 + width, c0, c0]
    elif orient==2:  #down
...
class RushHour(gym.Env):
    ...
    def __init__(self, width=5, height=5, render_type='cubes', num_objects=5,
        ...
        # x ,y ,orientations (0-3) same as ["forward"   ,"right", "backward", "left"]
        self.objects = [[-1, -1, -1] for _ in range(self.num_objects)]
    ...
    def render(self):
        if self.render_type == 'grid':
            ...
            for idx, pos in enumerate(self.objects):
                rr, cc = triangle(
                    pos[0]*10, pos[1]*10, 10, im.shape, orient=pos[2])
                im[rr, cc, :] = self.colors[idx][:3]
            return im.transpose([2, 0, 1])
    ...
    def step(self, action):
        ...
        obj = action // 4
        direction = (self.objects[obj][2] + action) % 4  

        #print("move obj:",obj)
        #print("direction:",self.DIRECTIONS[direction])
